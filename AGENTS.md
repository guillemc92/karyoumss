...
| ADR-0018 | Permisos por rol en `backend-clinic`: analista/supervisor/admin derivados de `is_staff`/`is_superuser` (sin campo `role` nuevo, sin migración, sin sincronización con `backend-admin`), cierra SPEC-008 §6 con `SampleDetailView` (`GET`/`PATCH`/`DELETE` por id, `DELETE` admin-only) | `docs/adr/0018-permisos-rol-backend-clinic.md` |
